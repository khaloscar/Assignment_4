...
int main(int argc, char *argv[]) {
    ...
    double* f_y_ij = (double*)malloc(N*sizeof(double)); // These are used to efficiently pre-compute forces, want to avoid calloc -> memset inside loop is used to reset the arrays
    ...
    if (en_graphics)
    {
        ...
        for (int step = 0; step < nsteps; step++)
        {
            ...
            update_state(N, particles,
                        delta_t, G, epsilon,
                        f_x_ij, f_y_ij);
            // ptr swapping ensruing current state is updated
    ...
    log_result("timings.txt", runtime, N, version_name, nsteps, Nthreads);
    return 0;

}

// Gonna test better structure for cache locality
